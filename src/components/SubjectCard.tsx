...
export function SubjectCard({
  ...
  const progress = subject === 'Dashboard' ? getTotalProgress() : getProgressBySubject(subject);
    ...
    damping: 20
      ...
      <Link to={subject === 'Dashboard' ? '/dashboard' : `/subject/${subject}`} className={`subject-card subject-card-${variant} p-6 h-40 flex flex-col justify-between group relative overflow-hidden`}>
        ...
        <div className="absolute right-0 bottom-0 w-24 h-24 opacity-20 group-hover:opacity-30 transition-opacity duration-300">
          <div className="absolute w-2 h-2 rounded-full bg-white animate-pulse-slow" style={{
          top: '20%',
          left: '30%'
        }} />
          <div className="absolute w-1.5 h-1.5 rounded-full bg-white animate-pulse-slow" style={{
          top: '40%',
          left: '70%',
          animationDelay: '0.5s'
        }} />
          <div className="absolute w-1 h-1 rounded-full bg-white animate-pulse-slow" style={{
          top: '70%',
          left: '20%',
          animationDelay: '1s'
        }} />
        ...
        <div className="flex justify-between items-start relative z-10">
          ...
          <motion.span role="img" aria-label={subject} animate={{
          scale: [1, 1.1, 1],
          rotate: [0, 5, 0, -5, 0]
        }} transition={{
          duration: 5,
        ...
        <div className="mt-auto relative z-10">
          <ProgressBar progress={progress} variant={variant} showPercentage />
        </div>
      </Link>
    </motion.div>;
}