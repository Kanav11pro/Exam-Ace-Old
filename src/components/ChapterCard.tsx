...
export function ChapterCard({
  ...
  return <Link to={`/subject/${subject}/${encodeURIComponent(chapter)}`} className="chapter-card bg-white dark:bg-gray-800 p-4 h-40 flex flex-col justify-between hover:translate-y-[-5px] hover:shadow-xl transition-all">
      <div className="flex justify-between items-start">
        <h3 className="text-lg font-semibold line-clamp-2 pr-2">{chapter}</h3>
        <span role="img" aria-label={chapter} className="text-6xl">
