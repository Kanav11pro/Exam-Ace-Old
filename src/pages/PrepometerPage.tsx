...
import { Calculator, Atom, Beaker, Target, TrendingUp, BookOpen, Star, Trophy, Rocket } from 'lucide-react';
const PrepometerPage = () => {
  ...
  return <div className="relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <motion.div className="absolute -top-40 -right-40 w-80 h-80 bg-gradient-to-br from-primary/20 to-secondary/20 rounded-full blur-3xl" animate={{
        scale: [1, 1.2, 1],
        rotate: [0, 180, 360]
      }} transition={{
        duration: 20,
        ...
        <motion.div className="absolute -bottom-40 -left-40 w-80 h-80 bg-gradient-to-br from-secondary/20 to-accent/20 rounded-full blur-3xl" animate={{
        scale: [1.2, 1, 1.2],
        rotate: [360, 180, 0]
      }} transition={{
        duration: 25,
      ...
      <div className="container max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8 relative z-10">
        ...
        y: 0
          ...
          <motion.div className="inline-flex items-center gap-2 mb-6 px-4 py-2 bg-gradient-to-r from-primary/10 to-secondary/10 rounded-full border border-primary/20" whileHover={{
          scale: 1.05
        }} whileTap={{
          scale: 0.95
        }}>
        ...
        <motion.div variants={containerVariants} initial="hidden" animate="visible" className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          ...
          return <motion.div key={subject.key} variants={itemVariants} whileHover={{
            scale: 1.02,
            y: -5
          }} whileTap={{
            scale: 0.98
          }} className="relative">
                <Card className={`cursor-pointer transition-all duration-300 hover:shadow-2xl border-2 ${isSelected ? 'border-primary shadow-xl ring-4 ring-primary/20' : 'border-transparent hover:border-primary/30'} ${subject.bgPattern}`} onClick={() => setSelectedSubject(isSelected ? null : subject.key)}>
                  <CardHeader className="pb-4 bg-slate-950">
                    <div className="flex items-center justify-between">
                      <motion.div className={`p-3 rounded-xl bg-gradient-to-br ${subject.color} text-white shadow-lg`} whileHover={{
                    rotate: 360
                  }} transition={{
                    duration: 0.6
                  }}>
                      ...
                      <Badge variant={progress > 75 ? 'default' : progress > 50 ? 'secondary' : 'outline'} className="px-3 py-1 font-semibold">
                        {progress.toFixed(1)}%
                      </Badge>
                    </div>
                  ...
                  <CardContent className="pt-0 bg-slate-950">
                    <div className="space-y-4">
                      ...
                      <Button className={`w-full bg-gradient-to-r ${subject.color} hover:opacity-90 text-white font-medium shadow-lg`} onClick={e => {
                    e.stopPropagation();
                ...
                <motion.div className="absolute -top-2 -right-2 w-6 h-6 bg-gradient-to-br from-yellow-400 to-orange-500 rounded-full" animate={{
              y: [0, -10, 0],
              rotate: [0, 180, 360]
            }} transition={{
              duration: 3,
